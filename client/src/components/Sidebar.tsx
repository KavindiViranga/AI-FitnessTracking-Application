    ...
    border-slate-100 dark:border-slate-800 p-6 transition-colors duration-200">
        ...
        <div className='flex flex-col gap-2'>
            {navItems.map((item) => (
                ...
                className={({isActive}) => `flex items-center gap-3 px-4 py-2.5 border-l-3 
                transition-al duration-200 ${isActive ? 'bg-emerald-50 dark:bg-emerald-900/10 text-emerald-600 dark:text-emerald-400 font-medium': 
        ...
        <div className='mt-auto pt-6 border-t border-slate-100 dark:border-slate-800'>
            ...
            rounded-lg transition-colors duration-200 cursor-pointer">
                {theme === 'light' ? <MoonIcon className='size-5' /> : <SunIcon 
                className='size-5' /> }
